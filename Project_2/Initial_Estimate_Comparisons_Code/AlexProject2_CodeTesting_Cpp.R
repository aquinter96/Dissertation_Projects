      ...
      for(a in 1:length(tuningpA)){
        for(b in 1:length(tuningpB)){
          ...
          while(((norm(A1 - A1CV, type = "F") > 0.001) | (norm(A2 - A2CV, type = "F") > 0.001) | (norm(B1 - B1CV, type = "F") > 0.001) | (norm(B2 - B2CV, type = "F") > 0.001) | (norm(PhiR - PhiRCV, type = "F") > 0.001) | (norm(PhiS1 - PhiS1CV, type = "F") > 0.001) | (norm(PhiS2 - PhiS2CV, type = "F") > 0.001) | (norm(Phi11 - Phi11CV, type = "F") > 0.001) | (norm(Phi12 - Phi12CV, type = "F") > 0.001)) & (maxit <= 10000)){
            ...
            A1CV <- coordesc_constr(n, A1, B1, as.matrix(Xpanel[[1]]), ER, ES1, condvarR, condvarRS1, Phi11, tuningpA[a], weightsA[[1]])
            # A2CV <- coordesc_full(n, A2, B2, as.matrix(Xpanel[[2]]), ER, ES2, condvarR, condvarRS2, Phi12, tuningpA[a], weightsA[[2]])
            # B1CV <- coordesc_constr(n, B1, A1, as.matrix(Xpanel[[1]]), ES1, ER, condvarS1, t(condvarRS1), Phi11, tuningpB[b], weightsB[[1]])
            # B2CV <- coordesc_constr(n, B2, A2, as.matrix(Xpanel[[2]]), ES2, ER, condvarS2, t(condvarRS2), Phi12, tuningpB[b], weightsB[[2]])
