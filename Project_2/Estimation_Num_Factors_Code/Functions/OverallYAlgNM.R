...
OverallYAlg <- function(Data, Xest, j_seq = 1:4, z_seq = list(1:4, 1:4)){
  ...
  #   for(b in 1:length(z_grid)){
      ...
      # Yest <- neldermead(tuningvals, EMAlg_Y, lower = rep(0, 4), Data = Data, Xest = Xest, initial_Model_Params = Yinit)
      
      Yinits <- parLapply(cl, lat_grid, Y_inits, Data, Xest)
      Yestlist <- parLapply(cl, Yinits, NMWrapperY, Data, Xest, tuningvals)
      
      # fmsfundata <- structure(list(Data = Data, Xest = Xest, initial_Model_Params = Yinit), class = 'optimbase.functionargs')
      # nm <- neldermead()
      # nm <- neldermead.set(nm,'numberofvariables',4)
      # nm <- neldermead.set(nm,'function',EMAlg_Y)
      # nm <- neldermead.set(nm,'x0',tuningvals)
      ...
      # nm <- neldermead.set(nm,'boundsmin',rep(0, 4))
      # nm <- neldermead.set(nm,'boundsmax',rep(2, 4))
      # nm <- neldermead.search(this = nm)
      # summary(nm)
      # Yest <-Nelder_Mead(EMAlg_Y, tuningvals, lower = rep(0, 4))
      # Yestlist[[c]] <- Yest
      # BIC_opt[c] <- Yest$value
