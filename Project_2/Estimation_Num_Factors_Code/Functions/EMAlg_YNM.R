...
EMAlg_Y <- function(tuningvals, Data, Xest, initial_Model_Params, NM = T){
  
  alg.start <- Sys.time()
  
  weights <- initial_Model_Params
  
  tuningpG1 <- tuningvals[1]
  tuningpD1 <- tuningvals[2]
  tuningpG2 <- tuningvals[3]
  tuningpD2 <- tuningvals[4]
