...
Xest <- OverallXAlg(MyData, m_seq = 1:5, u_seq = list(1:5, 1:5))
...
Yest <- OverallYAlg(MyData, Xest, j_seq = 1:5, z_seq = list(1:5, 1:5))

stopCluster(cl)

})

Yest$tot.time <- tot.time

## end of code
saveRDS(Yest, paste0("NMestlatrep", datset, "n", sampsize, ".rds"))