...
Mstep_X <- function(Data, Old_Par, E_estimates, tuningpA1, tuningpB1, tuningpA2, tuningpB2, weights){
  ...
  if(m == 1){
    condvarRS1 <- rbind(condvar[(1:m), (m+(1:u1))])
    condvarRS2 <- rbind(condvar[(1:m), (m+u1+(1:u2))])
  }else{
  ...
  if(m == 1){
    ER <- t(as.matrix(ERS[1:m,]))
  }else{
    ER <- ERS[1:m,]
  }
  
  if(u1 == 1){
    ES1 <- t(as.matrix(ERS[m+(1:u1),]))
  }else{
    ES1 <- ERS[m+(1:u1),]
  }
  
  if(u2 == 1){
    ES2 <- t(as.matrix(ERS[m+u1+(1:u2),]))
  }else{
    ES2 <- ERS[m+u1+(1:u2),]
  }
  ...
  for(i in 1:q1){
    for(j in 1:m){
      ...
      if(i > j){
        ...
        omegaijA1 <- (tcrossprod(ER) + n*condvarR)[-j,j]%*%as.matrix(A1[i,-j])
        tauijA1 <-  c(t(ER%*%t(ES1) + n*condvarRS1)[,j])%*%c(as.matrix(B1[i,]))
        a1bar <- ((thetaijA1-omegaijA1-tauijA1))/(epsilonijA1)
        if(as.matrix(weights$A1)[i,j] == 0){
          lambdaA <- (tuningpA1*Phi11[i,i])/epsilonijA1*(1/1e-5)
        }
        else{
          lambdaA <- (tuningpA1*Phi11[i,i])/epsilonijA1*(1/abs(as.matrix(weights$A1)[i,j]))
        }
  ...
  for(i in 1:q2){
    for(j in 1:m){
      ...
      omegaijA2 <- (tcrossprod(ER) + n*condvarR)[-j,j]%*%as.matrix(A2[i,-j])
      tauijA2 <- c(t(ER%*%t(ES2) + n*condvarRS2)[,j])%*%c(as.matrix(B2[i,]))
      a2bar <- ((thetaijA2-omegaijA2-tauijA2))/(epsilonijA2)
      if(as.matrix(weights$A2)[i,j] == 0){
        lambdaA <- (tuningpA2*Phi12[i,i])/epsilonijA2*(1/1e-5)
      }
      else{
        lambdaA <- (tuningpA2*Phi12[i,i])/epsilonijA2*(1/abs(as.matrix(weights$A2)[i,j]))
      }
  ...
  for(i in 1:q1){
    for(j in 1:u1){
      if(i > j){
        ...
        omegaijB1 <- (tcrossprod(ES1) + n*condvarS1)[-j,j]%*%as.matrix(B1[i,-j])
        tauijB1 <- c(t(ES1%*%t(ER) + n*t(condvarRS1))[,j])%*%c(A1[i,])
        b1bar <- ((thetaijB1-omegaijB1-tauijB1))/(epsilonijB1)
        if(as.matrix(weights$B1)[i,j] == 0){
          lambdaB <- (tuningpB1*Phi11[i,i])/epsilonijB1*(1/1e-5)
        }
        else{
          lambdaB <- (tuningpB1*Phi11[i,i])/epsilonijB1*(1/abs(as.matrix(weights$B1)[i,j]))
        }
  ...
  for(i in 1:q2){
    for(j in 1:u2){
      ...
      if(i > j){
        ...
        omegaijB2 <- (tcrossprod(ES2) + n*condvarS2)[-j,j]%*%as.matrix(B2[i,-j])
        tauijB2 <- c(t(ES2%*%t(ER) + n*t(condvarRS2))[,j])%*%c(A2[i,])
        b2bar <- ((thetaijB2-omegaijB2-tauijB2))/(epsilonijB2)
        if(as.matrix(weights$B2)[i,j] == 0){
          lambdaB <- (tuningpB2*Phi12[i,i])/epsilonijB2*(1/1e-5)
        }
        else{
          lambdaB <- (tuningpB2*Phi12[i,i])/epsilonijB2*(1/abs(as.matrix(weights$B2)[i,j]))
        }
  ...
  #################################################################################
  if(m == 1){
    PhiRnew <- 1/n*(n*condvarR + ER%*%t(ER))
  }else{
    PhiRnew <- 1/n*diag(diag(n*condvarR + ER%*%t(ER)))
  }
  if(u1 == 1){
    PhiS1new <- 1/n*(n*condvarS1 + ES1%*%t(ES1))
  }else{
    PhiS1new <- 1/n*diag(diag(n*condvarS1 + ES1%*%t(ES1)))
  }
  if(u2 == 1){
    PhiS2new <- 1/n*(n*condvarS2 + ES2%*%t(ES2))
  }else{
