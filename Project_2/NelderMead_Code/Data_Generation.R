...
Data_Generation <- function(n = 1000, Model_Params){
  ...
  Data$X1 <- t(MyPar$A1%*%Data$R + MyPar$B1%*%Data$S1 + t(MASS::mvrnorm(n, rep(0, q1), MyPar$Phi11))) ## X1
  Data$X2 <- t(MyPar$A2%*%Data$R + MyPar$B2%*%Data$S2 + t(MASS::mvrnorm(n, rep(0, q2), MyPar$Phi12))) ## X2 
  ...
  Data$Y1 <- t(MyPar$Gamma1%*%Data$V + MyPar$Delta1%*%Data$W1 + t(MASS::mvrnorm(n, rep(0, p1), MyPar$Phi21)))
  Data$Y2 <- t(MyPar$Gamma2%*%Data$V + MyPar$Delta2%*%Data$W2 + t(MASS::mvrnorm(n, rep(0, p2), MyPar$Phi22)))
