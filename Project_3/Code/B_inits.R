...
B_inits <- function(X, s){
  ...
  while(((norm(B0 - B0new, type = "F") > 0.00001) | (norm(B - Bnew, type = "F") > 0.00001) | (norm(Phi1 - Phi1new, type = "F") > 0.00001)) & (niter < 5000)){
    ...
    EV <- t(B)%*%invmodv%*%(t(X)-matrix(B0, nrow = ncol(X), ncol = n, byrow = F))
    B0new <- (1/n)*as.matrix(colSums(X - t(B%*%EV)))
    Bnew <- (crossprod(X,t(EV)) - matrix(B0new, nrow = q, ncol = n, byrow = F)%*%t(EV))%*%solve(n*condvar + tcrossprod(EV))
    Phi1new <- (1/n)*diag(diag(t(X)%*%X - 2*t(X)%*%matrix(B0new, nrow = n, ncol = q, byrow = T) - 2*t(X)%*%t(EV)%*%t(Bnew) + matrix(B0new, nrow = q, ncol = n, byrow = F)%*%t(matrix(B0new, nrow = q, ncol = n, byrow = F)) + 
                                 2*matrix(B0new, nrow = q, ncol = n, byrow = F)%*%t(EV)%*%t(Bnew) + n*Bnew%*%condvar%*%t(Bnew) + Bnew%*%EV%*%t(EV)%*%t(Bnew)))
    niter <- niter + 1
  }
  B0 <- B0new
  Phi1 <- Phi1new
  ...
  if(s==1){
    ...
    Psi <- diag(diag(B5[(1:s),(1:s)]))%*%diag(diag(B5[(1:s),(1:s)]))
