...
#' B FOR CONVERGENCE WHEN PERFORMING THE GRID SEARCH
  ...
  while(( (norm(B0 - B0new, type = "F") > 0.00001) | (norm(B - Bnew, type = "F") > 0.00001) | (norm(Phi1 - Phi1new, type = "F") > 0.00001) ) & (niter < 15000)){
    ...
    if(niter == 0){
      ...
      for(a in 1:length(tuningpB)){
        ...
        while((norm(B - BCV, type = "F") > 0.00001) & (maxit < 15000)){
            ...
            for(i in 1:xk_sig){
              for(j in 1:xk_sig){
                ...
                if(j < i){
                  ...
                  if(weights[i,j] == 0){
                    lambdaB <- (tuningpB[a]*Phi1[i,i])/deltaijB*(1/1e-5)
                  }
                  else{
                    lambdaB <- (tuningpB[a]*Phi1[i,i])/deltaijB*(1/abs(weights[i,j]))
                  }
        ...
        invmodcv <- Matrix::solve(Phi1 + tcrossprod(BCV))
