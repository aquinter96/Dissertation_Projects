...
#'
#' THIS CODE IS FOR THE ORIGINAL VERSION OF THE X EM ALGORITHM WHERE NONE
...
EMAlgBAdLassoCV <- function(X, xk_sig, Binit, Phi1init, tuningpB = seq(5.1,5.5,0.1), nfolds=10, weights){
  ...
  while(((max(abs(norm(B0, type="F")-norm(B0new, type="F"))) > 0.00001) | (max(abs(norm(B, type="F")-norm(Bnew, type="F"))) > 0.00001) | (max(abs(norm(Phi1, type="F")-norm(Phi1new, type="F"))) > 0.00001)) & (niter < 1000)){
    B0 <- B0new
    B <- Bnew
    Phi1 <- Phi1new
    ...
    if(niter == 0){
      ...
      for(a in 1:length(tuningpB)){
        lambdaB <- 0 #for each new tuning parameter we are using, reinitialize the coordinate descent updates to prevent carry over of computation
        ...
        thetaijB <- 0
        omegaijB <- 0
        while(abs(Bnorm - updateBnorm) > 0.00001){
          Bnorm <- norm(BCV, type = "F") #calculate Frobenius norm of B estimate from previous coordinate descent iteration
          invmodcv <- Matrix::solve(Phi1CV + tcrossprod(BCV))
          condvar <- diag(xk_sig) - crossprod(BCV,invmodcv)%*%BCV
          EW <- crossprod(BCV,invmodcv)%*%(t(X)-matrix(B0, nrow = q, ncol = n, byrow = F))
          for(i in 1:xk_sig){
            for(j in 1:xk_sig){
              deltaijB <- 0
              thetaijB <- 0
          ...
          Phi1CV <- (1/n)*diag(diag(t(X)%*%X - 2*t(X)%*%matrix(B0, nrow = n, ncol = q, byrow = T) - 2*t(X)%*%t(EW)%*%t(BCV) + matrix(B0, nrow = q, ncol = n, byrow = F)%*%t(matrix(B0, nrow = q, ncol = n, byrow = F)) + 
                                      2*matrix(B0, nrow = q, ncol = n, byrow = F)%*%t(EW)%*%t(BCV) + n*BCV%*%condvar%*%t(BCV) + BCV%*%EW%*%t(EW)%*%t(BCV)))
          updateBnorm <- norm(BCV, type = "F")
        }
      ...
      if(length(tuningpBhat) > 1){
        tuningpBhat <- min(tuningpBhat)
      }
