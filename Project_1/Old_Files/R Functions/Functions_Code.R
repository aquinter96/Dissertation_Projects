...
initvalcalc <- function(X, Y, Binit, s, m, n){
  ...
  Aobj <-function(a){
    ...
    norm((varY - AGamma%*%t(AGamma) - amat%*%t(amat) - phi2mat), type = "F")
  }
  ...
  while(((norm(A0 - A0new) > 0.00001) | (norm(A - Anew) > 0.00001) | (norm(Gamma - Gammanew) > 0.00001) | (norm(Phi2 - Phi2new) > 0.00001) | (norm(Phi3 - Phi3new) > 0.00001)) & (niter < 15000)){
    ...
    if(niter == 0){
      ...
      for(a in 1:length(tuningpA)){
        ...
        log.lik <- -(n/2)*(sum(diag(inv11%*%(varXY+(colMeans(cbind(X,Y)) - colMeans(cbind(X,Y)))%*%t(colMeans(cbind(X,Y)) - colMeans(cbind(X,Y)))))) + (q+p)*log((2*pi))+log(det(sigma11)))
