...
params1As <- matrix(0, nrow = 100, ncol = 3)
diag(params1As[(1:3),(1:3)]) <- 1
params1As[(1:3),(1:3)][lower.tri(params1As[(1:3),(1:3)], diag = F)] <- rbinom(3,1,0.5)*(2*rbinom(3,1,0.5)-1)*runif(3, 0.35, 1)
params1As[4:100,] <- rbinom(291,1,0.5)*(2*rbinom(291,1,0.5)-1)*runif(291, 0.35, 1)
params1G <- matrix(c(0.60, -0.75, -0.70, 0.65, 0.80, -0.55, 0.60, -0.40, 0.55), nrow = 3, ncol = 3)
q <- 100
p <- 100
s <- 3
m <- 3
...
Bres <- OverallBAlg(dat$X, s_seq = 3, tuningpB = 0)
Results <- OverallAGAlg(dat$X, dat$Y, Bres, m_seq = 3, tuningpA = 0)