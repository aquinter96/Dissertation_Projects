...
arma::mat coordesc(int n, arma::mat AB, arma::vec AB0, arma::mat XY, arma::mat EW, arma::mat condvar, arma::mat Phi12, double tuningp, arma::mat weights) {
  ...
  double delta = 0, theta = 0, omega = 0, lambdaAB = 0, abbar = 0, phi12val = 0, ABnorm = 1000;
  arma::vec inter (n), XYrow (n), AB0row (n);
  arma::rowvec EWrow (n);
  arma::mat secmom = EW*EW.t() + condvar, ABupdate = AB;
  while(ABnorm > 0.001 & iter < 10000){
    ABupdate = AB;
    for(int i = 0; i < a; ++i){
      XYrow = XY.col(i);
    ...
    ABnorm = arma::norm(AB-ABupdate, "fro");
    iter += 1;
  }
  return AB;
}
